...
int main() {
	...
	cout << endl << "Dijhkstra early path from 0 to 5: ";
	for (auto& element : early)
	{
		cout << element << " <- ";
	}
	cout << endl;
	...
	cout << "Bellman-Ford algorithm test" << endl;

	Graph digraphBell(3);
	path.resize(3);
	dist.resize(3);
	digraphBell.addWeightedEdge(0, 1, 4);
	digraphBell.addWeightedEdge(0, 2, 5);
	digraphBell.addWeightedEdge(2, 1, -3);

	cout << "Before negative cycle" << endl;
	digraphBell.BellmanFord(0, path, dist);
	for (int i = 0; i < 3; ++i) {
		cout << "Distance from 0 to " << i << " is " << dist[i] << endl;
	}

	digraphBell.addWeightedEdge(1, 0, -3);
	cout << "After negative cycle" << endl;
	digraphBell.BellmanFord(0, path, dist);
	for (int i = 0; i < 3; ++i) {
		cout << "Distance from 0 to " << i << " is " << dist[i] << endl;
	}
